...
#End Region
    ...
    Public Sub PermisosUsuario(ByVal vOpcion As String)
        ...
        Else
            ...
            Select Case vOpcion
                Case "Agregar"
                    Session("id_linea_act") = 0
                    Response.Redirect("~/LineasServicioAgregar.aspx")
                Case "Modificar"
                    If RadGrid1.SelectedItems.Count = 0 Then
                        scriptString = "alert('Favor de seleccionar una Línea de Servicio.');"
                        ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Error", scriptString, True)
                        Exit Sub
                    End If
                    If item("id_linea_servicio").Text = 99 Then
                        scriptString = "alert('No se puede modificar la Línea de Servicio 99.');"
                        ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Error", scriptString, True)
                        Exit Sub
                    End If
                    If item("ss").Text = "Baja" Then
                        scriptString = "alert('No se puede modificar la Línea de Servicio seleccionada porque es Baja.');"
                        ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Error", scriptString, True)
                        Exit Sub
                    End If
                    Session("id_linea_act") = item("id_linea_servicio").Text
                    Response.Redirect("~/LineasServicioAgregar.aspx")
                Case "Aplicar Baja"
                    If RadGrid1.SelectedItems.Count = 0 Then
                        scriptString = "alert('Favor de seleccionar una Línea de Servicio.');"
                    ...
                    End If
                    If item("id_linea_servicio").Text = 99 Then
                        scriptString = "alert('No se puede aplicar baja a la Línea de Servicio 99.');"
                        ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Error", scriptString, True)
                        Exit Sub
                    End If
                    Session("id_linea_act") = item("id_linea_servicio").Text
    ...
    Public Sub AplicarBaja(item As GridDataItem)
        ...
        Try
            Dim v_id_selec As Integer = Val(item("id_linea_servicio").Text)
            Dim v_estatus As String = item("ss").Text

            If v_estatus = "Baja" Then
                scriptString = "alert('La Línea de Servicio seleccionada ya es Baja.');"
                ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Error", scriptString, True)
                Exit Sub
            End If
            ...
            selSql = "SET DATEFORMAT dmy;" _
                & " BEGIN TRAN " _
                & "     " _
                & "     UPDATE LineasServicio SET ss = 0 " _
                & "	    WHERE id_linea_servicio = " & v_id_selec & "; " _
                & "     " _
                & "     INSERT INTO Bitacora VALUES ( '" & Today & "', '" & Format(Now, "hh:mm:ss") & "', '" & Usuario & "', 'Aplicó baja a la Línea de Servicio ID ' + '" & v_id_selec & "' , 0, 0)" _
                & "     " _
                & "		IF @@ERROR <> 0  " _
                & "		BEGIN " _
                & "			ROLLBACK " _
        ...
        Catch ex As Exception
            accessSQL.Close()
            scriptString = "alert('Error al intentar aplicar la baja a la Línea de Servicio seleccionada. \n\n " & ex.Message & " \n\n " & vError & "');"
            ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Error", scriptString, True)
        End Try
