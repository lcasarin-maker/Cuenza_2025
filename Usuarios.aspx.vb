...
#End Region
    ...
    Public Sub PermisosUsuario(ByVal vOpcion As String)
        ...
        Else
            ...
            Select Case vOpcion
                Case "Agregar"
                    Session("id_usuario_act") = 0
                    Response.Redirect("~/UsuariosAgregar.aspx")
                Case "Modificar"
                    If RadGrid1.SelectedItems.Count = 0 Then
                        scriptString = "alert('Favor de seleccionar un usuario.');"
                        ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Error", scriptString, True)
                        Exit Sub
                    End If
                    If item("ss").Text = "Baja" Then
                        scriptString = "alert('No se puede modificar el Usuario seleccionado porque es Baja.');"
                        ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Error", scriptString, True)
                        Exit Sub
                    End If
                    Session("id_usuario_act") = item("id_usuario").Text
                    Response.Redirect("~/UsuariosAgregar.aspx")
                Case "Aplicar Baja"
                    If RadGrid1.SelectedItems.Count = 0 Then
                        scriptString = "alert('Favor de seleccionar un usuario.');"
    ...
    Public Sub AplicarBaja(item As GridDataItem)
        ...
        Try
            Dim v_id_selec As Integer = Val(item("id_usuario").Text)
            Dim v_estatus As String = item("ss").Text

            If v_estatus = "Baja" Then
                scriptString = "alert('El usuario seleccionado ya es Baja.');"
                ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Error", scriptString, True)
                Exit Sub
            End If

            'scriptString = "if(!confirm('¿Deseas aplicar baja al usuario seleccionado?')) { return false; }"
            ...
            selSql = "SET DATEFORMAT dmy;" _
                & " BEGIN TRAN " _
                & "     " _
                & "     UPDATE Usuarios SET status_sistema = 0 " _
                & "	    WHERE id_usuario = " & v_id_selec & "; " _
                & "     " _
                & "     INSERT INTO Bitacora VALUES ( '" & Today & "', '" & Format(Now, "hh:mm:ss") & "', '" & Usuario & "', 'Aplicó baja al usuario ID ' + '" & v_id_selec & "' , 0, 0)" _
                & "     " _
                & "		IF @@ERROR <> 0  " _
                & "		BEGIN " _
                & "			ROLLBACK " _
        ...
        Catch ex As Exception
            accessSQL.Close()
            scriptString = "alert('Error al intentar aplicar la baja al usuario seleccionado. \n\n " & ex.Message & " \n\n " & vError & "');"
            ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Error", scriptString, True)
        End Try
