...
#End Region
    ...
    Public Sub PermisosUsuario(ByVal vOpcion As String)
        ...
        Else
            ...
            Select Case vOpcion
                Case "Agregar"
                    Session("id_servicio_act") = 0
                    Response.Redirect("~/ServiciosAgregar.aspx")
                Case "Modificar"
                    If RadGrid1.SelectedItems.Count = 0 Then
                        scriptString = "alert('Favor de seleccionar un Servicio.');"
                        ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Error", scriptString, True)
                        Exit Sub
                    End If
                    If item("ss").Text = "Baja" Then
                        scriptString = "alert('No se puede modificar el Servicio seleccionado porque es Baja.');"
                        ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Error", scriptString, True)
                        Exit Sub
                    End If
                    Session("id_servicio_act") = item("id_servicio").Text
                    Response.Redirect("~/ServiciosAgregar.aspx")
                Case "Aplicar Baja"
                    If RadGrid1.SelectedItems.Count = 0 Then
                        scriptString = "alert('Favor de seleccionar un Servicio.');"
    ...
    Public Sub AplicarBaja(item As GridDataItem)
        ...
        Try
            Dim v_id_selec As Integer = Val(item("id_servicio").Text)
            Dim v_estatus As String = item("ss").Text

            If v_estatus = "Baja" Then
                scriptString = "alert('El Servicio seleccionado ya es Baja.');"
                ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Error", scriptString, True)
                Exit Sub
            End If
            ...
            selSql = "SET DATEFORMAT dmy;" _
                & " BEGIN TRAN " _
                & "     " _
                & "     UPDATE Servicios SET ss = 0 " _
                & "	    WHERE id_servicio = " & v_id_selec & "; " _
                & "     " _
                & "     INSERT INTO Bitacora VALUES ( '" & Today & "', '" & Format(Now, "hh:mm:ss") & "', '" & Usuario & "', 'Aplicó baja al Servicio ID ' + '" & v_id_selec & "' , 0, 0)" _
                & "     " _
                & "		IF @@ERROR <> 0  " _
                & "		BEGIN " _
                & "			ROLLBACK " _
        ...
        Catch ex As Exception
            accessSQL.Close()
            scriptString = "alert('Error al intentar aplicar la baja al Servicio seleccionado. \n\n " & ex.Message & " \n\n " & vError & "');"
            ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Error", scriptString, True)
        End Try
