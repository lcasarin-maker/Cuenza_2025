...
Partial Class PronosticoProgramaModificar
...
#Region "Variables"
    ...
    Shared id_cliente, id_servicio, id_status, id_status_desc, id_ingreso_pk As Integer
    Shared facturas_generadas As Integer
...
#End Region
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            ...
            If Not Session("usuario") Is Nothing Then
                ...
                facturas_generadas = Session("facturas_generaddas")
    ...
    Public Sub LlenarDatos()
        ...
        ' Datos del pronóstico-programa o gasto
        ...
        id_empresa = v_DataTable.Rows(0).Item("id_empresa")
        id_cliente = v_DataTable.Rows(0).Item("id_cliente")
        id_servicio = v_DataTable.Rows(0).Item("id_servicio")
        id_status = v_DataTable.Rows(0).Item("id_status")
        id_status_desc = v_DataTable.Rows(0).Item("id_status_desc")

        LabelTitulo.Text = IIf(id_status = 1, "Modificar Pronóstico", IIf(id_status = 2, "Modificar Programa", "Modificar Gasto"))
        '1	Pronóstico (No Confirmado)
        '2	Programa (Confirmado)
        '3	Gasto

        RadNumericTextBoxNoPagos1.Text = v_DataTable.Rows(0).Item("total_pagos")
        RadNumericTextBoxNoPagos2.Text = v_DataTable.Rows(0).Item("total_pagos")
        RadNumericTextBoxFolio.Text = v_DataTable.Rows(0).Item("total_pagos")
        ...
        RadComboBoxQuienFactura.SelectedIndex = RadComboBoxQuienFactura.FindItemIndexByText(v_DataTable.Rows(0).Item("nombre_empresa"))
        RadComboBoxCliente.SelectedIndex = RadComboBoxCliente.FindItemIndexByText(v_DataTable.Rows(0).Item("nombre_cliente"))
        ...
        RadComboBoxTipoMoneda.SelectedIndex = RadComboBoxTipoMoneda.FindItemIndexByText(v_DataTable.Rows(0).Item("descripcion_tipo_moneda"))

        RadTextBoxDetalle.Text = v_DataTable.Rows(0).Item("detalle")

        Dim selectedItem2 As New RadComboBoxItem()
        selectedItem2.Text = v_DataTable.Rows(0).Item("descripcion_status")
        selectedItem2.Value = v_DataTable.Rows(0).Item("descripcion_status")
        RadComboBoxStatus.Items.Add(selectedItem2)

        RadTextBoxEspecificaciones.Text = v_DataTable.Rows(0).Item("especificacion_pago")

        Dim vStatusDesc As String = v_DataTable.Rows(0).Item("descripcion_status_desc")

        Dim vServicio As String = v_DataTable.Rows(0).Item("nombre_servicio")
        SessionServicio()
        RadComboBoxServicio.SelectedIndex = RadComboBoxServicio.FindItemIndexByText(vServicio)

        StatusDesc()

        RadComboBoxStatusDesc.SelectedIndex = RadComboBoxStatusDesc.FindItemIndexByText(vStatusDesc)
        ...
        selSql = "CREATE TABLE " & TableName & " ([tipo_factura] Nvarchar(MAX), [no_pago] [int] NOT NULL, [folio_factura] [int] NOT NULL, [fecha_fact] [date] NOT NULL, [fecha_pago] [date] NOT NULL, [importe] [decimal](18, 2) NOT NULL, [concepto] [nvarchar](max) NOT NULL, [importe_letra] [nvarchar](max) NULL) "
        accessSQL.ExecuteNonQueryD(selSql)
        accessSQL.Close()

        oPag.InsertarIngresosPagosIDTmpBL(TableName, id_ingreso_pk)
    ...
    Public Sub HabilitarControles()
        If facturas_generadas = 0 Then ' No tiene facturas generadas
            RadComboBoxLineaServicio.Enabled = True
            RadComboBoxQuienFactura.Enabled = True
            RadComboBoxCliente.Enabled = True
            RadComboBoxServicio.Enabled = True
            RadComboBoxTipoMoneda.Enabled = True
            RadComboBoxStatus.Enabled = True
        Else
            RadComboBoxLineaServicio.Enabled = False
            RadComboBoxQuienFactura.Enabled = False
            RadComboBoxCliente.Enabled = False
            RadComboBoxServicio.Enabled = False
            RadComboBoxTipoMoneda.Enabled = False
    ...
    Public Sub AgregarPago()
        ...
        If RadGridPagos.Items.Count > 0 Then
            Dim vTotalPagos As Integer = 0

            For Each dataItem As GridDataItem In RadGridPagos.MasterTableView.Items
                Dim vTipoFactura As String = dataItem("tipo_factura").Text
                If vTipoFactura <> "Cancelada" Then
                    vTotalPagos = vTotalPagos + 1
                End If
            Next

            If vNoPagos2 = vTotalPagos Then
        ...
        selSql = "SET DATEFORMAT dmy; INSERT INTO " & TableName & " VALUES ( 'No Facturada', " & RadNumericTextBoxNoPagos1.Value & ", " & RadNumericTextBoxFolio.Value & ", '" & RadDatePickerFechaFacturacion.SelectedDate & "', '" & RadDatePickerFechaPago.SelectedDate & "', " & vImportePago & ", '" & RadTextBoxConcepto.Text & "', '" & RadNumericTextBoxImportePagoLetra.Text & "')"
        accessSQL.ExecuteNonQueryD(selSql)
        accessSQL.Close()

        ' Ordernar por fecha de facturación
        selSql = "SET DATEFORMAT dmy; " _
        & "       SELECT tipo_factura " _
        & "            , no_pago = ROW_NUMBER() OVER (ORDER BY fecha_fact) " _
        & "            , folio_factura " _
    ...
    Public Sub ModificarPago()
        ...
        ' Validar si el pago tiene tipo de factura generada, pagada o cancelada
        'Dim vTipoFac As Integer = item("tipo_factura").Text
        If item("tipo_factura").Text <> "No Facturada" Then
            scriptString = "alert('Solo puede modificar pagos con tipo de factura No Facturada.');"
            ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Error", scriptString, True)
            Exit Sub
        End If
        ...
        RadNumericTextBoxImportePagoLetra.Text = Replace(item("importe_letra").Text, "&nbsp;", "")
        ...
        'RadToolBarAgregarPagos.Visible = False
        'RadToolBarModificarPagos.Visible = False
        'RadToolBarEliminarPagos.Visible = False
        'RadToolBarGrabarPagos.Visible = True
    ...
    Protected Sub ButtonEliminar_Click(sender As Object, e As EventArgs)
        ...
        ' Validar si el pago tiene tipo de factura generada, pagada o cancelada
        'Dim vTipoFac As Integer = item("id_tipo_factura").Text
        If item("tipo_factura").Text <> "No Facturada" Then
            scriptString = "alert('Solo puede eliminar pagos con tipo de factura No Facturada.');"
            ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Error", scriptString, True)
            Exit Sub
        End If
    ...
    Public Sub CancelarPago()
        ...
        selSql = "SET DATEFORMAT dmy; INSERT INTO " & TableName & " VALUES ( '" & item("tipo_factura").Text & "', " & item("no_pago").Text & ", " & item("folio_factura").Text & ", '" & item("fecha_fact").Text & "', '" & item("fecha_pago").Text & "', " & item("importe").Text & ", '" & item("concepto").Text & "', '" & item("importe_letra").Text & "')"
        accessSQL.ExecuteNonQueryD(selSql)
        accessSQL.Close()
        ...
        'RadToolBarGrabarPagos.Visible = False
        'RadToolBarAgregarPagos.Visible = True
        'RadToolBarModificarPagos.Visible = True
        'RadToolBarEliminarPagos.Visible = True
    ...
    Public Sub ModificarPronostico()
        ...
        Try
            ...
            'RadNumericTextBoxNoPagos1.Value = RadGridPagos.Items.Count
            ...
            '--------------------------------------------------------------------------------
            ' Actualizar e insertar información en tablas de la base de datos
            '--------------------------------------------------------------------------------
            selSql = "SET DATEFORMAT dmy;" _
                & " BEGIN TRAN " _
                & "     " _
                & "     UPDATE Ingresos SET " _
                & "	                id_linea_servicio = " & id_linea_servicio & " " _
                & "	               ,id_empresa = " & id_empresa & " " _
                & "	               ,id_cliente = " & id_cliente & " " _
                & "	               ,id_servicio = " & id_servicio & " " _
                & "	               ,años = '" & v_años & "' " _
                & "	               ,id_status = " & id_status & " " _
                & "	               ,id_status_desc = " & id_status_desc & " " _
                & "	               ,importe = " & vImporte & " " _
                & "	               ,detalle = '" & RadTextBoxDetalle.Text & "' " _
                & "	               ,total_pagos = " & vTotalPagos & " " _
                & "	               ,especificacion_pago = '" & RadTextBoxEspecificaciones.Text & "' " _
                & "	               ,id_tipo_moneda = " & id_tipo_moneda & "  " _
                & "     WHERE id_ingreso_pk = " & id_ingreso_pk _
                & "     " _
                & "     DELETE IngresosPagos WHERE id_ingreso_pk = " & id_ingreso_pk & ";" _
                & "     " _
                ...
                & "	          ,id_tipo_factura     = TiposFactura.id_tipo_factura " _
                & "	          ,pago_1              = 0 " _
                & "	          ,pago_2              = 0 " _
                & "	          ,pago_3              = 0 " _
                & "	          ,pago_4              = 0 " _
                ...
                & "     INNER JOIN TiposFactura ON (PAG.tipo_factura = TiposFactura.descripcion) " _
                & "     ORDER BY PAG.no_pago; " _
                & "     " _
                & "     Drop Table " & TableName & "; " _
                & "     " _
                & "     INSERT INTO Bitacora VALUES ( '" & Today & "', '" & Format(Now, "hh:mm:ss") & "', '" & Usuario & "', 'Modificó " & v_status & "', " & id_ingreso & ", " & id_linea_servicio & ")" _
                & "     " _
                & "		IF @@ERROR <> 0  " _
                & "		BEGIN " _
                & "			ROLLBACK " _
            ...
            oAle.AlertaModificoProBL(v_status, urlTemplate, id_ingreso, id_linea_servicio, id_empresa, id_cliente, id_servicio, vImporte, n_ls, n_e, n_c, n_s)
