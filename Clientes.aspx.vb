...
#End Region
    ...
    Public Sub PermisosUsuario(ByVal vOpcion As String)
        ...
        Select Case vOpcion
            Case "Agregar"
                ...
                '    Exit Sub
                'End If
                Session("id_cliente_act") = 0
                Response.Redirect("~/ClientesAgregar.aspx")
            Case "Modificar"
                'If id_Usuario > 0 Then
                '    scriptString = "alert('Usuario sin permisos para la opción seleccionada.');"
                '    ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Error", scriptString, True)
                ...
                'End If
                If RadGrid1.SelectedItems.Count = 0 Then
                    scriptString = "alert('Favor de seleccionar un Cliente.');"
                    ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Error", scriptString, True)
                    Exit Sub
                End If
                If item("ss1").Text = "Baja" Then
                    scriptString = "alert('No se puede modificar el Cliente seleccionado porque es Baja.');"
                    ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Error", scriptString, True)
                    Exit Sub
                End If
                Session("id_cliente_act") = item("id_cliente").Text
                Response.Redirect("~/ClientesAgregar.aspx")
            Case "Aplicar Baja"
                'If id_Usuario > 0 Then
                '    scriptString = "alert('Usuario sin permisos para la opción seleccionada.');"
    ...
    Public Sub AplicarBaja(item As GridDataItem)
        ...
        Try
            Dim v_id_selec As Integer = Val(item("id_cliente").Text)
            Dim v_estatus As String = item("ss1").Text

            If v_estatus = "Baja" Then
                scriptString = "alert('El Cliente seleccionado ya es Baja.');"
                ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Error", scriptString, True)
                Exit Sub
            End If
            ...
            selSql = "SET DATEFORMAT dmy;" _
                & " BEGIN TRAN " _
                & "     " _
                & "     UPDATE Clientes SET ss = 0 " _
                & "	    WHERE id_cliente = " & v_id_selec & "; " _
                & "     " _
                & "     INSERT INTO Bitacora VALUES ( '" & Today & "', '" & Format(Now, "hh:mm:ss") & "', '" & Usuario & "', 'Aplicó baja al Cliente ID ' + '" & v_id_selec & "' , 0, 0)" _
                & "     " _
                & "		IF @@ERROR <> 0  " _
                & "		BEGIN " _
                & "			ROLLBACK " _
        ...
        Catch ex As Exception
            accessSQL.Close()
            scriptString = "alert('Error al intentar aplicar la baja al Cliente seleccionado. \n\n " & ex.Message & " \n\n " & vError & "');"
            ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Error", scriptString, True)
        End Try
